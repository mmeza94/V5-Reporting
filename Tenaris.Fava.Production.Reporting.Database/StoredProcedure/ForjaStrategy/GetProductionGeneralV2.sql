USE [L2_Forja]
GO
/****** Object:  StoredProcedure [dbo].[GetProductionGeneralV2TestNuevo]    Script Date: 20/01/2023 10:20:16 a. m. ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
If OBJECT_ID('[dbo].[GetProductionGeneralV2TestNuevo] ') IS NOT NULL
	DROP PROCEDURE [dbo].[GetProductionGeneralV2TestNuevo]
GO




ALTER PROCEDURE [dbo].[GetProductionGeneralV2TestNuevo] 
...
DECLARE @TEMP2 
	...
	DECLARE @TEMPInicial 
		...
		idHistory INT
		,LoadedCount INT
		--,LoadadCountProgrammed INT
		--,LoadedCountCalculado INT
		,GoodCount INT
		,WarnedCount INT 
		,ScrapCount INT 
		,StandardCount INT 
...
WHERE (HS.LoadedCount > 0) 	AND
	  (@Orden = 0 OR Production.OrderHistory.OrderNumber = @Orden) AND
	  (@Colada = 0 OR HH.HeatNumber = @Colada) AND
	  (@Atado = 0 OR GIH.GroupItemNumber = @Atado) 
	  --AND MN.Description like '%Forjadora 0%'
...
SELECT @Forja0Buenos  = ISNULL(SUM(GoodCount),0) FROM @TEMPInicial WHERE [Description]='Bancal de buenas de Forjadora 0'
SELECT @Forja0Malos1 = ISNULL(SUM(GoodCount),0) FROM @TEMPInicial WHERE [Description]='Cuneta de descartes 1 de Forjadora 0'
SELECT @Forja0Malos2 = ISNULL(SUM(GoodCount),0) FROM @TEMPInicial WHERE [Description]='Cuneta de descartes 2 de Forjadora 0'
SELECT @Forja0ReprocesosBuenos = ISNULL(SUM(ReworkedCount),0) FROM @TEMPInicial WHERE [Description]='Bancal de buenas de Forjadora 0'
...
SELECT *,ROW_NUMBER() OVER (Partition by GroupItemNumber Order by idHistory) as ReportSequence
FROM @TEMPInicial
Where Description like 'Forjadora 0' and LoadedCount >= (SELECT MAX(LoadedCount) FROM @TEMPInicial WHERE [Description] = 'Forjadora 0')

--EXCEPT
--	SELECT * 
--	FROM @TEMPInicial
--	WHERE [Description] = 'Forjadora 0'
--	AND LoadedCount < (SELECT MAX(LoadedCount) FROM @TEMPInicial WHERE [Description] = 'Forjadora 0')
